...
%%
	...
	Gdecl 	:	ret_type Glist ';'  {/*create head for this linked list here .*/}
		;
		
	ret_type:	T_INT		{ /*make rrty gb var as 1 , so that we know this waas int */}
		|	T_BOOL 		{ }
		;
		
	Glist 	:	Gid	{/*attach this to glist*/}
		| 	func 	{}
		|	Gid ',' Glist 
		|	func ',' Glist
		;
	
	Gid	:	VAR		{ /*keep counting the num if 0 then var else array .*/				}
		|	Gid '[' NUM ']'	{                                                   }

		;
		
	func 	:	VAR '(' arg_list ')' 					{ 	/*create node and push to stack */				}
		;
